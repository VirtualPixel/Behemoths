...
- **Behemoth monsters.** Every monster on a boss level carries more health, hits harder, and shrugs off a share of incoming damage, so it takes real punishment to bring down. That part the whole lobby feels, mod or no mod. On top of that they grow, 1.4x by default, body included: what you see is what you shoot, grab, and get hit by, and the swing reaches as far as it looks. Height holds at vanilla so they still fit doorways, and pathing keeps its vanilla width so they always find the door. Push SizeMultiplier higher for more bulk, or hold the body back with the caps where a map's doors need it. (The size is drawn by ScalerCore, so only players running the mods see it; see below.)
...
- **Boss orbs.** The orb a Behemoth drops is worth 2.5x the cash of a normal one and tough enough to take a beating without losing its value, both of which everyone in the lobby gets. For players running the mods it is also twice the size (heavy to match, a real haul) and amber-glowing. It never shrinks back down.
...
Only the host installs this, so it is worth being straight about what reaches everyone else. The danger and the payout are the whole lobby's: bosses take far more hits to kill, hit harder (the extra damage rides the game's own damage path to every player), shrug off damage, and every orb and valuable is worth more (the cash values sync like any other loot). Nobody needs the mod for any of that.

What needs the mod to be seen is the spectacle. The size of the monsters and their orbs is drawn by ScalerCore, which only paints scale on machines that also run it, so a guest with nothing installed sees a vanilla-sized monster that just happens to be a tank. The amber orb glow and the camera tremors are host-only. If you want the crew to see them tower and glow, everyone runs the mod (it pulls ScalerCore with it); if you just want the fight to be harder and pay better, the host alone is enough.
